...
/// A matchable wrapper for the device, exported without feature-gating the variants
/// so that downstream crates can reference them in match arms.
pub enum SurffaceDevice {
    Cuda(Box<dyn std::any::Any>),
    Wgpu(Box<dyn std::any::Any>),
    Cpu(Box<dyn std::any::Any>),
}

/// The dispatch function that bridges the dynamic device selection to the
/// static execute_stages call.
pub fn dispatch<F>(f: F)
where
    F: FnOnce(SurffaceDevice),
{
    #[cfg(feature = "cuda")]
    {
        f(SurffaceDevice::Cuda(Box::new(
...
pub fn get_device() -> <AutoBackend as Backend>::Device {
    #[cfg(feature = "cuda")]
    {
        info!("🚀 Using CUDA backend (NVIDIA GPU)");
        burn_cuda::CudaDevice::default()
    }

    #[cfg(all(feature = "wgpu", not(feature = "cuda")))]
    {
        info!("🎮 Using WGPU backend (GPU via Vulkan/Metal/DX12)");
        burn_wgpu::WgpuDevice::default()
    }
